...
class Workspace:
	...
	def create_version(
		...
		self.n_versions += 1
		new_version_dir_path = self.resolve_path(self._get_version_dirname(str(self.n_versions)))

		os.makedirs((f'version_{self.n_versions}'), exist_ok=True)
	...
	def view(
	    ...
	    if recursive:
	        for path in abs_paths:
	            if os.path.isdir(path):  # If it's a directory, walk it
	                ...
	                all_files[path] = None
	    else:  # Only process files in paths
	        for path in abs_paths:
