...
class PicoGPTClimber(ScienceRunner):
	...
	def set_results_for_version(self, version: str, job_results: slurm_utils.JobResult):
		...
		metrics = {}
		matches = re.findall(r"epoch:(\d+)(?:/\d+)?\s+train loss:([\d.]+)\s+test loss:([\d.]+)\s+test acc:([\d.]+)", log_out)
		if matches:
		    # Take the last match
		    last_match = matches[-1]
		    metrics = {
			    "test_loss": float(last_match[2]),
		        "test_acc": float(last_match[3]),
		    }
...
async def main():
	if len(sys.argv) != 2:
	    print("Usage: python climb_picogpt.py <vllm server node_id>")
	    sys.exit(1)
	...
	root_path = 'workspaces/picogpt' + f'_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}'
	template_dir = 'workspace_templates/picogpt'
	workspace = Workspace(root_path=root_path, template_dir=template_dir)

	exp_config = ExperimentConfig(
		preamble=prompts.TASK_PREAMBLE,
		job_ttl=1*30  # Run for a maximum of 30 min
	)

	climber = PicoGPTClimber(config=exp_config, workspace=workspace, scientist=scientist)
